...
export default function MasterAdminPage() {
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      fetchGeneratedWorkOrders().catch(() => undefined)
      fetchRetryQueue().catch(() => undefined)
      ...
      <div className="space-y-6">
        ...
        {activeSection === "employees" && (
          <div className="space-y-4">
            ...
            <form onSubmit={handleCreateEmployee} className="rounded-lg border border-gray-200 bg-white p-5 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  <Label>Employee ID</Label>
                  <Input value={employeeForm.employee_id} onChange={(e) => setEmployeeForm((prev) => ({ ...prev, employee_id: e.target.value }))} required />
                </div>
                <div>
                  <Label>Full Name</Label>
                  <Input value={employeeForm.full_name} onChange={(e) => setEmployeeForm((prev) => ({ ...prev, full_name: e.target.value }))} required />
                </div>
                <div>
                  <Label>Login ID</Label>
                  <Input value={employeeForm.login_id} onChange={(e) => setEmployeeForm((prev) => ({ ...prev, login_id: e.target.value }))} required />
                </div>
                <div>
                  <Label>Password</Label>
                  <Input type="password" value={employeeForm.password} onChange={(e) => setEmployeeForm((prev) => ({ ...prev, password: e.target.value }))} required />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <Input value={employeeForm.contact_number} onChange={(e) => setEmployeeForm((prev) => ({ ...prev, contact_number: e.target.value }))} />
                </div>
                <div>
                  <Label>Email</Label>
                  <Input type="email" value={employeeForm.email} onChange={(e) => setEmployeeForm((prev) => ({ ...prev, email: e.target.value }))} />
                </div>
                <div>
                  <Label>Designation</Label>
                  <Input value={employeeForm.designation} onChange={(e) => setEmployeeForm((prev) => ({ ...prev, designation: e.target.value }))} />
                </div>
              </div>
            ...
            <div className="rounded-lg border border-gray-200 bg-white overflow-x-auto">
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {employees.length === 0 ? (
                    <tr>
                      <td className="px-3 py-3" colSpan={5}>No employees found.</td>
                    </tr>
                  ) : (
                    employees.map((employee) => (
                      <tr key={employee.id} className="border-t">
                        <td className="px-3 py-2">{employee.employee_id}</td>
                        <td className="px-3 py-2">{employee.full_name}</td>
                        <td className="px-3 py-2">{employee.login_id}</td>
